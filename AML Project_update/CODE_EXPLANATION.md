...
def _create_llm():
    if GEMINI_API_KEY:
        ...
        fallback = ChatOpenAI(model="gemini-3.5-flash-lite", ...)
        return primary.with_fallbacks([fallback])
    return ChatOpenAI(model="gpt-4o-mini", ...)
```
Uses Google Gemini via the **OpenAI-compatible endpoint** — no separate SDK needed. Falls back to `gemini-3.5-flash-lite` automatically if the primary model is overloaded.
