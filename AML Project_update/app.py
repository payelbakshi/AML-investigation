...
def get_adjudication_tier(score) -> tuple[str, str]:
    try:
        ...
        val = 0
    if val >= 70:
        return "🔴 Draft STR (High Risk)", "high"
    elif val >= 35:
        return "🟡 Escalate for Human Review", "medium"
    else:
        return "🟢 Close (False Positive)", "low"
...
def run_investigation(
    ...
    if run_aml_pipeline is None:
        message = "Pipeline unavailable. Configure Gemini (GEMINI_API_KEY) or OpenAI (OPENAI_API_KEY) API key to run agents."
        return (
    ...
    try:
        ...
        if not is_quota_error:
            raise
        message = (
            "API quota exhausted or invalid key. Check your Gemini/OpenAI API key and "
            "available quota, then retry the investigation."
        )
        return (
            "Pipeline unavailable",
            message,
