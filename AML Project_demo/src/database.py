...
def format_kyc_context(customer_id: str) -> str:
    ...
    if risk_score >= 70:
        triage_status = "🔴 HIGH PRIORITY (Review for STR / Confirmed Anomaly)"
    elif risk_score >= 35:
        triage_status = "🟡 MEDIUM PRIORITY (Review Required / Elevated Discrepancy)"
    else:
        triage_status = "🟢 LOW PRIORITY (Routine / KYC-Aligned)"
    ...
    return (
        ...
        f"• Initial Triage Baseline: {triage_status}"
    )
